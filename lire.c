...
int main(int argc, char *argv[])
{
    ...
    char fichieraudio[100]; // on crée le tableau pour stocker le nom du fichier audio
    char fichierdat[105]; 
    //float filtre_freq; 
    //float filtre_large;
    ...
    sprintf (fichierdat,"%s.dat", fichieraudio); // stocker le nom du fichier . dat dans le tableau fichierdat
    ...
    struct wavfile header; //creation du header = pour stocker toutes les données

    if ( wav == NULL ) // on vérifie si l'ouverture fonctionne 
    {
    ...
    if (    header.id[0] != 'R'|| header.id[1] != 'I'|| header.id[2] != 'F'|| header.id[3] != 'F' ) // format valide ?
    {
        printf("\nerreur le fichier n'est pas un format wave valide\n");
        exit(1);
    }
    if (header.channels!=1) // on veut un fichier mono
    {
    ...
    nbech=(header.bytes_in_data/header.bytes_by_capture); //nombre d’échantillons contenus dans la partie DATA
    ...
    if (data == NULL) // on vérifie si le malloc a fonctionné
    {
        exit(0);
    }

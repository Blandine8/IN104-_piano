...
int main(int argc, char *argv[])
{
    ...
    if ( fread(&header,sizeof(header),1,wav) < 1 ) // sizeof(&header)=sizeof(header)*1, lit le fichier wav le stocke dans header et
    {
...
//    FILE *dat2=fopen("fabs_son.dat","w");//fichier.dat des valeurs absolues des echantillons
    ...
    while( fread(&value,(header.bits_per_sample)/8,1,wav)) //fread(&value,(header.bits_per_sample)/8,1,wav) 
    {
